...
public class ImageUtils {
	 ...
	 * @param name
	 */
	public static void saveNetwokImage(String url, String folder, String name) {
		InputStream ins = getInputStreamFromUrl(url, 0);
		StringBuilder filePath = new StringBuilder(folder);
		filePath.append("/").append(name);
		if (!name.contains(".jpg")) {
			filePath.append(".jpg");
		}
		FileUtils.writeFile(filePath.toString(), ins);
	}
