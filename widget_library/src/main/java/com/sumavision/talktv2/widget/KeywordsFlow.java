...
public class KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener {
	...
	public boolean feedKeyword(List<String> keywords) {
		boolean result = false;
		if (keywords != null && keywords.size() > 0) {
			if (keywords.size() >= MAX) {
				result = vecKeywords.addAll(keywords.subList(0, MAX));
			} else {
				result = vecKeywords.addAll(keywords);
			}
		}
		return result;
	}
	...
	private void attach2Screen(LinkedList<TextView> listTxt, int xCenter,
		...
		for (int i = 0; i < size; i++) {
			...
			if (layParams.leftMargin < 0) {
				layParams.leftMargin = 2;
			}
	...
	// float def = Math.abs(ev.getX() - startX);
	// Log.e("def", String.valueOf(def));
