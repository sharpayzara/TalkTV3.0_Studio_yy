 ...
 * @version 2.2
 * @createTime 2013-1-8
 * @description 用户发表的评论解析类
 * @changLog
 */
public class UserTalkListParser extends BaseJsonParser {
	public ArrayList<CommentData> talkList = new ArrayList<CommentData>();
	...
	public void parse(JSONObject jAData) {
		...
		try {
			...
			if (errCode == JSONMessageType.SERVER_CODE_OK) {
				...
				isVip = jsonObject.optInt("isVip")==1 ? true:false;
				talkCount = jsonObject.getInt("talkCount");
				UserNow.current().commentCount = talkCount;
				if (talkCount > 0) {
					...
					for (int i = 0; i < content.length(); i++) {
						...
						c.topicID = item.optLong("topicId");
						...
						c.isAnonymousUser = item.getJSONObject("user").getInt(
								"isAnonymousUser");
						c.isVip = item.getJSONObject("user").optInt("isVip")==1?true:false;
						talkList.add(c);
					}
				}
			}
